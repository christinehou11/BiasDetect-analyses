...
clusters1 <- read.csv(
    here("processed-data","spatialHPC_SRT","seuInt-hpc_k-7_svgs_metadata.csv"), 
    row.names=1)
clusters2 <- read.csv(
    here("processed-data","spatialHPC_SRT",
        "seuInt-hpc_k-7_svgs-no-bias_metadata.csv"), 
    row.names=1)
...
spe$precast_k7_nobias_ordered <- factor(spe$precast_k7_nobias, 
...
png(here("plots", "spatialHPC_SRT","cluster_heatmap_with_bias.png"), 
    width=5, height=5, units="in", res=300)
p1_heat <- plotGroupedHeatmap(spe, features = markers, 
...
png(here("plots", "spatialHPC_SRT","precast_cluster_bias.png"), 
    width=5, height=5, units="in", res=300)
