...
# Figure - nSD_rank
sd.interval = 6
batch_df_sub6$nSD.bin_rank = cut(abs(batch_df_sub6$nSD_rank), right=FALSE,
                             breaks=seq(0,max(batch_df_sub6$nSD_rank)+sd.interval, 
                                        by=sd.interval),
                             include.lowest=TRUE)

col.pal3 = RColorBrewer::brewer.pal(length(unique(batch_df_sub6$nSD.bin_rank)), "YlOrRd")
col.pal3[1] = "grey"

png(here("plots", "spatialHPC_SRT","SVGs_nSD_rank_sub6.png"))
...
bias_sub6 <- BiasDetect::biasDetect(batch_df = batch_df_sub6, nSD_dev = 5, nSD_rank = 5)

# display bias in plots
bias_plots_sub6 <- BiasDetect::biasDetect(batch_df = batch_df_sub6, 
                        nSD_dev = 5, nSD_rank = 5, visual = TRUE)
# deviance
